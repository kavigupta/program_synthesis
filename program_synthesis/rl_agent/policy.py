...
from models.modules import karel
...
class KarelEditPolicy(nn.Module):
    ...
    def __init__(self, vocab_size: int, args):
        ...
        # Coding input
        self.task_encoder = karel.make_task_encoder(args)
        self.code_encoder = karel.CodeEncoderRL(args)
        self.token_embed = nn.Embedding(vocab_size, 256)

        print(self.task_encoder)
        print(self.code_encoder)
        print(self.token_embed)
        # Getting output
    ...
    def get_parameters_tensors(self, codes_enc, tasks_enc, actions):
        ...
        for idx, action in enumerate(actions):
            ...
            if action.id == mutation.ADD_ACTION:
                ...
                raise ValueError("action.id must lie in the range [0, 8) found {}".format(action.id))
        ...
        return (torch.cat(param_value), values_list)
...
class BaseModel(nn.Module):
    ...
    def _assert_size(param, size):
        if isinstance(size, tuple):
            assert len(param.size()) == len(size), "Expected tensor of dimension {} found {}".format(size,param.size())

            for ps, rs in zip(param.size(), size):
                assert rs is None or ps == rs, "Expected tensor of dimension {} found {}".format(size,param.size())

        else:
            assert len(param.size()) == size, "Expected tensor of dimension {} found {} ({})".format(size,len(param.size()),param.size())
    ...
    def _assert_batch_size(*params):
        ...
        assert ok, "Expected first dimension to coincide but found {}".format([p.size() for p in params])
